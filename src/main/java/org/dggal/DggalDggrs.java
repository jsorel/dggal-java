...
public final class DggalDggrs implements AutoCloseable {
    ...
    public double[] getZoneRefinedWGS84Vertices(long zone, int refinement) throws Throwable {
        try (final DggalArrayGeoPoint array = new DggalArrayGeoPoint(dggal,
                (MemorySegment) dggal.DGGAL_DGGRS_getZoneRefinedWGS84Vertices.invokeExact(pointer, zone, refinement))) {
            return array.toArray();
        }
    }
    ...
    public long[] getSubZones(long zone, int depth) throws Throwable {
        try (final DggalArrayZone array = new DggalArrayZone(dggal,
                (MemorySegment) dggal.DGGAL_DGGRS_getSubZones.invokeExact(pointer, zone, depth))) {
            return array.toArray();
        }
    }
    ...
    public long[] compactZones(long[] zones) throws Throwable {
        try (Arena tempArena = Arena.ofConfined()){
            final MemorySegment mem = tempArena.allocate((zones.length+1)*8);
            LongBuffer buffer = mem.asByteBuffer().order(ByteOrder.LITTLE_ENDIAN).asLongBuffer();
            buffer.put(zones.length); //array end
            buffer.put(zones);
            final MemorySegment address = tempArena.allocateFrom(ValueLayout.JAVA_LONG, mem.address());
            final DggalArrayZone gp = new DggalArrayZone(dggal, address);
            dggal.DGGAL_DGGRS_compactZones.invokeExact(pointer, address);
            return gp.toArray();
        }
    }
